...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete_cat') {
        ...
        if ($count->fetchColumn() > 0) {
            header('Location: ' . BASE . '/categories.php?error=' . urlencode('Cannot delete: category has products')); exit;
        }
        $db->prepare("DELETE FROM categories WHERE id=?")->execute([$cid]);
        header('Location: ' . BASE . '/categories.php?success=' . urlencode('Category deleted')); exit;
    }
    ...
    if ($action === 'delete_unit') {
        ...
        $abbr->execute([$uid]); $abbr_val = $abbr->fetchColumn();
        $count = $db->prepare("SELECT COUNT(*) FROM products WHERE unit_type=?");
        $count->execute([$abbr_val]);
        if ($count->fetchColumn() > 0) {
            header('Location: ' . BASE . '/categories.php?tab=units&error=' . urlencode('Cannot delete: unit is used by products')); exit;
        }
        $db->prepare("DELETE FROM units WHERE id=?")->execute([$uid]);
        header('Location: ' . BASE . '/categories.php?tab=units&success=' . urlencode('Unit deleted')); exit;
    }
    if ($action === 'toggle_unit') {
        $db->prepare("UPDATE units SET is_active = 1 - is_active WHERE id=?")->execute([(int)$_POST['id']]);
        header('Location: ' . BASE . '/categories.php?tab=units&success=' . urlencode('Status updated')); exit;
    }
}

// ── Load Data ──
$all_cats = $db->query("
    SELECT c.id, c.name, COALESCE(c.name_ar,'') as name_ar,
           COALESCE(c.emoji,'📦') as emoji,
           COALESCE(c.description,'') as description,
           c.parent_id, COALESCE(c.is_active,1) as is_active,
           pc.name as parent_name, COALESCE(pc.emoji,'📦') as parent_emoji,
...
$parent_cats = array_values(array_filter($all_cats, fn($c) => !$c['parent_id']));
$units = $db->query("
    SELECT u.id, u.name, COALESCE(u.name_ar,'') as name_ar,
           u.abbreviation, COALESCE(u.is_active,1) as is_active,
           (SELECT COUNT(*) FROM products WHERE unit_type=u.abbreviation) as product_count
...
<!-- ── CATEGORIES TAB ── -->
<div id="tab-categories" <?= $active_tab==='units'?'style="display:none"':'' ?>>
  <div class="stats-grid" style="grid-template-columns:repeat(3,1fr)">
    ...
    <div class="stat-card green"><div class="stat-label">Main</div><div class="stat-value text-green"><?= count($parent_cats) ?></div></div>
    <div class="stat-card purple"><div class="stat-label">Sub</div><div class="stat-value text-accent"><?= count($all_cats) - count($parent_cats) ?></div></div>
  </div>

  <div class="card">
    <div class="card-title">
      <span>📂 <?= __('all_categories') ?></span>
      <button class="btn btn-primary btn-sm" onclick="openAddCat()">+ <?= __('add_category') ?></button>
    </div>
    <div class="tbl-wrap">
      <table>
        <thead><tr>
          ...
          <tr>
            <td>
              <div style="display:flex;align-items:center;gap:8px">
                <span style="font-size:20px;flex-shrink:0"><?= htmlspecialchars($c['emoji']) ?></span>
                <div>
                  <div style="font-weight:500"><?= htmlspecialchars($c['name']) ?></div>
                  <?php if ($c['name_ar']): ?>
                  <div style="font-size:11px;color:var(--text3);direction:rtl"><?= htmlspecialchars($c['name_ar']) ?></div>
                  <?php endif; ?>
                  <?php if ($c['description']): ?>
                  <div style="font-size:11px;color:var(--text3)"><?= htmlspecialchars($c['description']) ?></div>
                  <?php endif; ?>
                </div>
              </div>
            </td>
            <td class="hide-mobile"><span class="badge <?= $c['parent_id'] ? 'badge-blue' : 'badge-green' ?>"><?= $c['parent_id'] ? 'Sub' : 'Main' ?></span></td>
            <td class="hide-mobile"><?= $c['parent_name'] ? htmlspecialchars($c['parent_emoji'] . ' ' . $c['parent_name']) : '<span style="color:var(--text3)">—</span>' ?></td>
            <td style="font-weight:600"><?= $c['product_count'] ?></td>
            <td>
              <form method="POST" style="display:inline">
                <input type="hidden" name="action" value="toggle_cat">
                <input type="hidden" name="id" value="<?= $c['id'] ?>">
                <button type="submit" class="badge <?= $c['is_active'] ? 'badge-green' : 'badge-gray' ?>" style="cursor:pointer;border:none">
                  <span class="dot"></span><?= $c['is_active'] ? __('active') : __('inactive') ?>
                </button>
              </form>
            </td>
            <td>
              <div style="display:flex;gap:4px;flex-wrap:wrap">
                <button class="btn btn-ghost btn-sm" onclick='editCat(<?= htmlspecialchars(json_encode($c, JSON_HEX_APOS|JSON_HEX_QUOT)) ?>)'>✏️</button>
...
<!-- ── UNITS TAB ── -->
<div id="tab-units" <?= $active_tab!=='units'?'style="display:none"':'' ?>>
  <div class="stats-grid" style="grid-template-columns:repeat(3,1fr)">
  ...
  <div class="card">
    <div class="card-title">
      <span>📏 <?= __('units') ?></span>
      <button class="btn btn-primary btn-sm" onclick="openAddUnit()">+ <?= __('add_unit') ?></button>
    </div>
    <div class="tbl-wrap">
      <table>
        <thead><tr>
          ...
          <th class="hide-mobile"><?= __('total_products') ?></th>
          <th><?= __('status') ?></th>
          <th><?= __('actions') ?></th>
        </tr></thead>
        <tbody>
          ...
          <tr>
            <td>
              ...
              <?php if ($u['name_ar']): ?><div style="font-size:11px;color:var(--text3);direction:rtl"><?= htmlspecialchars($u['name_ar']) ?></div><?php endif; ?>
            </td>
            <td><span class="badge badge-blue"><?= htmlspecialchars($u['abbreviation']) ?></span></td>
            <td class="hide-mobile" style="font-weight:600"><?= $u['product_count'] ?></td>
            <td>
              <form method="POST" style="display:inline">
                <input type="hidden" name="action" value="toggle_unit"><input type="hidden" name="id" value="<?= $u['id'] ?>">
                <button type="submit" class="badge <?= $u['is_active'] ? 'badge-green' : 'badge-gray' ?>" style="cursor:pointer;border:none">
            ...
            <td>
              <div style="display:flex;gap:4px">
                <button class="btn btn-ghost btn-sm" onclick='editUnit(<?= htmlspecialchars(json_encode($u, JSON_HEX_APOS|JSON_HEX_QUOT)) ?>)'>✏️</button>
      ...
      <div class="modal-body">
        <div class="form-row">
          <div class="form-group">
            <label class="form-label"><?= __('category_name') ?> (EN) *</label>
            <input class="form-input" name="name" id="cat-name" required placeholder="e.g. Bags">
          </div>
          <div class="form-group">
            <label class="form-label"><?= __('emoji') ?></label>
            <input class="form-input" name="emoji" id="cat-emoji" placeholder="📦" style="text-align:center;font-size:18px" maxlength="4">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label"><?= __('category_name') ?> (<?= __('arabic') ?>)</label>
          <input class="form-input" name="name_ar" id="cat-name-ar" placeholder="مثال: حقائب" style="direction:rtl">
        </div>
        <div class="form-group">
        ...
        <div class="form-group" id="cat-parent-group" style="display:none">
          ...
          <select class="form-select" name="parent_id" id="cat-parent">
            ...
            <option value="<?= $pc['id'] ?>"><?= htmlspecialchars($pc['emoji'] . ' ' . $pc['name']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="form-group">
          <label class="form-label"><?= __('description') ?></label>
          <input class="form-input" name="description" id="cat-desc" placeholder="Optional description">
        </div>
      </div>
...
<div class="modal-backdrop" id="unit-modal">
  <div class="modal" style="max-width:420px">
    ...
    <form method="POST" id="unit-form">
      ...
      <div class="modal-body">
        <div class="form-group"><label class="form-label"><?= __('unit_name') ?> (EN) *</label><input class="form-input" name="unit_name" id="unit-name" required placeholder="e.g. Carton"></div>
        <div class="form-group"><label class="form-label"><?= __('unit_name') ?> (<?= __('arabic') ?>)</label><input class="form-input" name="unit_name_ar" id="unit-name-ar" placeholder="كرتون" style="direction:rtl"></div>
...
function openAddCat() {
  ...
  document.getElementById("cat-action").value = "add_cat";
  document.getElementById("cat-id").value = "";
  document.getElementById("cat-parent-group").style.display = "none";
  document.getElementById("cat-modal-title").textContent = "' . __('add_category') . '";
  openModal("cat-modal");
}
function editCat(c) {
...
function openAddUnit() {
  ...
  document.getElementById("unit-action").value = "add_unit";
  document.getElementById("unit-id").value = "";
  document.getElementById("unit-modal-title").textContent = "' . __('add_unit') . '";
...
function editUnit(u) {
  ...
  document.getElementById("unit-name-ar").value = u.name_ar || "";
